...
from flask import make_response # this is method, convert return value into real response obj that can be sent to user 
from sqlalchemy import create_engine, desc
from sqlalchemy.orm import sessionmaker
from database_setup import Base, Item, Category, User
import random, string
import httplib2 # http client lib in python 
import json # covnert in-memory python object to serialized representation called JSON
import requests # Apache 2 licensed HTTP lib in python, similar to urllib2
from oauth2client.client import flow_from_clientsecrets #this methode creates flow obj from the clientssecrets JSON file
from oauth2client.client import FlowExchangeError # run into an error when trying to return access token
...
def createUser(login_session):
	newUser = User(username = login_session['username'], 
		email = login_session['email'])#, gplus_id = login_session['gplus_id'])
	session.add(newUser)
	session.commit()
